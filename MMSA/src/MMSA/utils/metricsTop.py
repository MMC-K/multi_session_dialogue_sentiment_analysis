...
class MetricsTop():
    ...
    @staticmethod
    def _safe_pearsonr(y_true, y_pred) -> float:
        import numpy as np
        ...
        if y_true.size < 2:
            return 0.0
        s1, s2 = np.std(y_true), np.std(y_pred)
        if s1 == 0.0 or s2 == 0.0:
            return 0.0
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        # MAE (원래대로)
        mae = np.mean(np.abs(test_preds - test_truth)).astype(np.float64)

        # 안전한 상관계수
        corr = self._safe_pearsonr(test_truth, test_preds)

        # --- 폴라리티 자동 보정(이진 지표에만 적용) ---
        # corr<0이면 예측 부호만 뒤집어 이진 지표 안정화. (MAE/Mult_acc/Corr은 원값 유지)
        signed_preds = test_preds if corr >= 0 else -test_preds

        # 멀티클래스 정확도(원래대로; 부호 보정 X)
        ...
        # Non-zero 이진 지표 (예측만 signed_preds 사용)
        non_zeros = np.array([i for i, e in enumerate(test_truth) if e != 0])
        non_zeros_binary_truth = (test_truth[non_zeros] > 0)
        non_zeros_binary_preds = (signed_preds[non_zeros] > 0)
        non_zeros_acc2 = accuracy_score(non_zeros_binary_preds, non_zeros_binary_truth)
        non_zeros_f1_score = f1_score(non_zeros_binary_truth, non_zeros_binary_preds, average='weighted')

        # Has0 이진 지표 (0 포함 / 예측만 signed_preds 사용)
        binary_truth = (test_truth >= 0)
        binary_preds = (signed_preds >= 0)
