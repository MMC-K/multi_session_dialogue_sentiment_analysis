    ...
    if is_tune: # run tune
        ...
        if config: # override some arguments
            if config.get('model_name'):
                assert(config['model_name'] == args['model_name'])
            args.update(config)
        args.setdefault('resume_weights', '/home/ujeong/tmp/MMSA_master/MMSA/cenet-mosei.pth')   # e.g., "/home/ujeong/tmp/MMSA_master/MMSA/cenet-mosei.pth"
        args.setdefault('resume_epoch', 9)      # e.g., 9 (1-based epoch index)
        args.setdefault('strict_resume', False) # True면 키 불일치시 에러
        # torch.cuda.set_device() encouraged by pytorch developer, although dicouraged in the doc.
        # https://github.com/pytorch/pytorch/issues/70404#issuecomment-1001113109
        # It solves the bug of RNN always running on gpu 0.
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    # ===== [RESUME PATCH] 9epoch 체크포인트에서 웜스타트 =====
    try:
        resume_ckpt = args.get('resume_weights', '')
        resume_epoch = int(args.get('resume_epoch', 0) or 0)  # 1-based (예: 9)

        if resume_ckpt:
            ckpt_path = Path(resume_ckpt)
            assert ckpt_path.exists(), f"Resume checkpoint not found: {ckpt_path}"
            state = torch.load(ckpt_path, map_location=args['device'])
            model.load_state_dict(state, strict=bool(args.get('strict_resume', False)))
            logger.info(f"✓ Loaded resume weights from {ckpt_path}")

            # 남은 에포크만 학습되도록 조정 (ex: 원래 12, resume_epoch=9 → 남은 3)
            if 'num_epochs' in args and resume_epoch > 0:
                original_num = int(args['num_epochs'])
                if resume_epoch >= original_num:
                    logger.warning(f"resume_epoch({resume_epoch}) >= num_epochs({original_num}); set num_epochs=0")
                    args['num_epochs'] = 0
                else:
                    args['num_epochs'] = original_num - resume_epoch
                logger.info(f"→ Resuming after epoch {resume_epoch}: {original_num} → remaining {args['num_epochs']} epochs")
            else:
                logger.info("→ Resume loaded; using configured num_epochs as-is.")
    except Exception as e:
        logger.exception(f"[RESUME PATCH] failed: {e}")
        raise
    # =========================================================
